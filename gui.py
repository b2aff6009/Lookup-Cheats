...
class GuiEntry:
    ...
    def __init__(self, entry, isHeadline, root, settings, x, mRow=0, mCol=0):
        ...
        for colNr, colEntry in enumerate(self.entry):
            ...
            self.cells.append(Label(self.frame, text=colEntry, bg=bgColor,
                                    bd=1, anchor=W, font=labelFont, wraplength=colWidth))
...
class Gui:
    def __init__(self, finder, settings={}, isSheetSelector=False):
        ...
        self.createSearchBar(self.root, self.windowWidth,
                             int(self.windowHeight/10), 0)

        self.frameWidth = self.windowWidth/self.settings["columns"]
        self.mainFrame = Frame(self.root, width=self.windowWidth, height=int(
            9*self.windowHeight/10), bg="white")
        ...
        if isSheetSelector:
            ...
            self.root.bind(
                self.settings["selectionDown"], self.changeSelection)
        else:
            colNr = self.settings["columns"]
            for i in range(0, colNr):
                self.mainFrames.append(Frame(self.mainFrame, width=self.frameWidth, height=int(
                    9*self.windowHeight/10), bg="white"))
                self.mainFrames[-1].grid(row=1, column=i)
                if not self.settings["multiLineEntry"]:
                    self.headlines.append(GuiEntry(
                        self.finder.order, True, self.mainFrames[-1], self.settings, self.frameWidth))
        ...
        if ("cleanKey" in self.settings.keys()):
            self.root.bind(self.settings["cleanKey"],
                           lambda x: self.searchBar.delete(0, 'end'))
    ...
    def createMainWindow(self):
        ...
        if (self.settings["Debug"] == True):
            ...
            print("X: {} Y: {} Height: {} Width: {}".format(
                self.positionX, self.positionY, self.windowHeight, self.windowWidth))
    ...
    def createEntry(self, key, hits):
        if (self.isSheetSelector):
            ...
            frame = self.mainFrames[len(
                self.loadedFrames.keys()) % self.settings["columns"]]
            self.loadedFrames[key] = GuiEntry(
                hits[key], False, frame, self.settings, self.frameWidth)

    def updateGui(self):
        hits = self.finder.find(self.searchBar.get())

        keys = list(filter(lambda x: x not in hits.keys(),
                           self.visibleFrames.keys()))
        for key in keys:
