...
def search_data():
    data = {"common": [
            ...
            {"shourtcut" : "ctrl+d", "data": "four", "anOtherEntry" : "This is a sentence"},
            {"shourtcut" : "ctrl+e", "data": "five", "anOtherEntry" : ["Using", "a", "list"]}
            ],
            "visible": ["data"]
    }
    return data
...
def test_standard_find_sheets():
    ...
    assert len(sheetFinder.find("CompleyString")) == 0, "Faild for complex string"
...
def test_search_result_standard():
    ...
    assert len(finder.find("")) == len(data["common"]), "Didn't found all entrys"
